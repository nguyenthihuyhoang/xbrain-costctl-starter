...
def _to_tags(set_args):
    ...
    return [{"Key": k, "Value": v} for k, v in (parse_kv(item) for item in set_args)]
...
def _tag_s3(rid, tags):
    ...
    try:
        ...
        if code not in ("NoSuchTagSet", "NoSuchBucketTagging", "NoSuchTagSetError"):
            raise
        existing = []
    ...
    s3.put_bucket_tagging(
        Bucket=rid,
        Tagging={"TagSet": [{"Key": k, "Value": v} for k, v in sorted(merged.items())]},
    )
